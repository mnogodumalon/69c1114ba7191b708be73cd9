...
const KUNDENABRECHNUNG_FIELDS = [
  { key: 'kunde_ref', label: 'Kunde', type: 'applookup/select', targetEntity: 'testeingabe', targetAppId: 'TESTEINGABE', displayField: 'vorname' },
  { key: 'rechnungsnummer', label: 'Rechnungsnummer', type: 'string/text' },
  { key: 'leistungsbeschreibung', label: 'Leistungsbeschreibung', type: 'string/text' },
  { key: 'menge', label: 'Menge', type: 'number' },
  { key: 'einzelpreis', label: 'Einzelpreis (EUR)', type: 'number' },
  { key: 'gesamtbetrag', label: 'Gesamtbetrag (EUR)', type: 'number' },
  { key: 'zahlungsart', label: 'Zahlungsart', type: 'lookup/select', options: [{ key: 'ueberweisung', label: 'Überweisung' }, { key: 'barzahlung', label: 'Barzahlung' }, { key: 'kreditkarte', label: 'Kreditkarte' }] },
  { key: 'faelligkeit', label: 'Fälligkeitsdatum', type: 'date/date' },
...
const ENTITY_TABS = [
  { key: 'kundenabrechnung', label: 'Kundenabrechnung', pascal: 'Kundenabrechnung' },
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<EntityKey>('kundenabrechnung');
  const [selectedIds, setSelectedIds] = useState<Record<EntityKey, Set<string>>>(() => ({
    kundenabrechnung: new Set(),
    testeingabe: new Set(),
  }));
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'kundenabrechnung': return (data as any).kundenabrechnung as Kundenabrechnung[] ?? [];
  ...
  const getLookupLists = useCallback((entity: EntityKey) => {
    ...
    switch (entity) {
      case 'kundenabrechnung':
        lists.testeingabeList = (data as any).testeingabe ?? [];
        break;
    }
    return lists;
  }, [data]);
  ...
  const getApplookupDisplay = useCallback((entity: EntityKey, fieldKey: string, url?: unknown) => {
    ...
    if (entity === 'kundenabrechnung' && fieldKey === 'kunde_ref') {
      const match = (lists.testeingabeList ?? []).find((r: any) => r.record_id === id);
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'kundenabrechnung': return KUNDENABRECHNUNG_FIELDS;
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'kundenabrechnung': return {
        create: (fields: any) => LivingAppsService.createKundenabrechnungEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateKundenabrechnungEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteKundenabrechnungEntry(id),
      };
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'kundenabrechnung' || dialogState?.entity === 'kundenabrechnung') && (
        <KundenabrechnungDialog
          open={createEntity === 'kundenabrechnung' || dialogState?.entity === 'kundenabrechnung'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'kundenabrechnung' ? handleUpdate : (fields: any) => handleCreate('kundenabrechnung', fields)}
          defaultValues={dialogState?.entity === 'kundenabrechnung' ? dialogState.record?.fields : undefined}
          testeingabeList={(data as any).testeingabe ?? []}
      ...
      {viewState?.entity === 'kundenabrechnung' && (
        <KundenabrechnungViewDialog
          open={viewState?.entity === 'kundenabrechnung'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'kundenabrechnung', record: r }); }}
          testeingabeList={(data as any).testeingabe ?? []}
        />
      )}
