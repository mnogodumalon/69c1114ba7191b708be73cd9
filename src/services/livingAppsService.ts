...
export class LivingAppsService {
  ...
  static async createTesteingabeEntry(fields: Testeingabe['fields']) {
    return callApi('POST', `/apps/${APP_IDS.TESTEINGABE}/records`, { fields });
  }
  static async updateTesteingabeEntry(id: string, fields: Partial<Testeingabe['fields']>) {
    return callApi('PATCH', `/apps/${APP_IDS.TESTEINGABE}/records/${id}`, { fields });
  }
  ...
  // --- KUNDENABRECHNUNG ---
  static async getKundenabrechnung(): Promise<Kundenabrechnung[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.KUNDENABRECHNUNG}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Kundenabrechnung[];
    return enrichLookupFields(records, 'kundenabrechnung');
  }
  static async getKundenabrechnungEntry(id: string): Promise<Kundenabrechnung | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.KUNDENABRECHNUNG}/records/${id}`);
    const record = { record_id: data.id, ...data } as Kundenabrechnung;
    return enrichLookupFields([record], 'kundenabrechnung')[0];
  }
  static async createKundenabrechnungEntry(fields: Kundenabrechnung['fields']) {
    return callApi('POST', `/apps/${APP_IDS.KUNDENABRECHNUNG}/records`, { fields });
  }
  static async updateKundenabrechnungEntry(id: string, fields: Partial<Kundenabrechnung['fields']>) {
    return callApi('PATCH', `/apps/${APP_IDS.KUNDENABRECHNUNG}/records/${id}`, { fields });
  }
  static async deleteKundenabrechnungEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.KUNDENABRECHNUNG}/records/${id}`);
  }
